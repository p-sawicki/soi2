 ...
 */
  register struct proc *temp, *temp_next;
  ...
  if (rdy_head[USER_Q] == NIL_PROC){
	rdy_tail[USER_Q] = rp;
  	...
  	if(rp->quotents_max == 0)
  		rp->quotents_max = 1;
  	if(rp->quotents_cur == 0)
  		rp->quotents_cur = rp->quotents_max;
  	rp->p_nextready = rdy_head[USER_Q];
  	rdy_head[USER_Q] = rp;
  }
  /*temp = rdy_head[USER_Q];
  while(temp->p_nextready != NIL_PROC && temp->p_nextready->quotents_cur > rp->quotents_cur)
  	temp = temp->p_nextready;
  temp_next = temp->p_nextready;
  if(temp_next == NIL_PROC){
 ...
 */
  ...
  register struct proc *prev, *temp, *temp_next;
  if (rdy_head[USER_Q] == NIL_PROC) return;

  /* One or more user processes queued. */
  prev = temp = rdy_head[USER_Q];
  --(prev->quotents_cur);
  if(prev->quotents_cur == 0){
  	prev->quotents_cur = prev->quotents_max;
  ...
  /*while(temp->p_nextready != NIL_PROC && temp->p_nextready->quotents_cur > prev->quotents_cur)
  	temp = temp->p_nextready;
  if(temp != prev){
  	temp_next = temp->p_nextready;
  	temp->p_nextready = rdy_head[USER_Q];
  	rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  	temp->p_nextready->p_nextready = temp_next;
  }*/
  pick_proc();
}
