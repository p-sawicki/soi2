...
FORWARD _PROTOTYPE( int do_setquotent, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETQUOTENT: r = do_setquotent(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->quotents_cur = rpc->quotents_max = rpp->quotents_max;
...
#endif /* (CHIP == INTEL) */
PRIVATE int do_setquotent(message *msg){
	register struct proc *p;
	for(p = BEG_USER_ADDR; p < END_PROC_ADDR; ++p)
		if(p->p_pid == msg->m1_i1)
			break;
	p->quotents_max = msg->m1_i2;
	p->quotents_cur = msg->m1_i2;
	return(OK);
}
